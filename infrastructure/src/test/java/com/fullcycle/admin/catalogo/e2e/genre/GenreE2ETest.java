...
public class GenreE2ETest implements MockDsl {
        ...
        public void asACatalogAdminIShouldBeAbleToCreateANewGenreWithValidValues()
                ...
                final var actualId =
                                givenAGenre(expectedName, expectedIsActive, expectedCategories);
        ...
        public void asACatalogAdminIShouldBeAbleToNavigateThruAllGenres() throws Exception {
                ...
                listGenres(0, 1).andExpect(status().isOk())
                ...
                listGenres(1, 1).andExpect(status().isOk())
                ...
                listGenres(2, 1).andExpect(status().isOk())
                ...
                listGenres(3, 1).andExpect(status().isOk())
        ...
        public void asACatalogAdminIShouldBeAbleToSearchBetweenAllGenres() throws Exception {
                ...
                listGenres(0, 1, "dra").andExpect(status().isOk())
                                .andExpect(jsonPath("$.current_page", equalTo(0)))
                                .andExpect(jsonPath("$.per_page", equalTo(1)))
        ...
        public void asACatalogAdminIShouldBeAbleToSortAllGenresByNameDesc() throws Exception {
                ...
                listGenres(0, 3, "", "name", "desc").andExpect(status().isOk())
                                .andExpect(jsonPath("$.current_page", equalTo(0)))
        ...
        @Test
        public void asACatalogAdminIShouldBeAbleToSeeATreatedErrorByGettingANotFoundGenre()
                        throws Exception {
                Assertions.assertTrue(MYSQL_CONTAINER.isRunning());
                Assertions.assertEquals(0, genreRepository.count());

                final var aRequest = get("/genres/123").with(ApiTest.ADMIN_JWT)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON);

                this.mvc.perform(aRequest).andExpect(status().isNotFound()).andExpect(
                                jsonPath("$.message", equalTo("Genre with ID 123 was not found")));
        }
        ...
        @Test
        public void asACatalogAdminIShouldBeAbleToInactivateAGenreByItsIdentifier()
                        throws Exception {
                Assertions.assertTrue(MYSQL_CONTAINER.isRunning());
                Assertions.assertEquals(0, genreRepository.count());
        ...
        @Test
        public void asACatalogAdminIShouldNotSeeAnErrorByDeletingANotExistentGenre()
                        throws Exception {
                Assertions.assertTrue(MYSQL_CONTAINER.isRunning());
                Assertions.assertEquals(0, genreRepository.count());
